...
export function TextRoll({ children, className = '', center = false }) {
  return <motion.span initial="initial" whileHover="hovered" className={`skiper-text-roll ${className}`}>
    <span>{children.split('').map((letter, index) => {
    ...
    <span className="skiper-text-roll-copy">{children.split('').map((letter, index) => {
      const delay = center ? STAGGER * Math.abs(index - (children.length - 1) / 2) : STAGGER * index
